...
describe("A5 Run Trace drawer (smoke)", () => {
  ...
  it("shows the COST stat between tokens and findings", () => {
    ...
    expect(within(label.parentElement as HTMLElement).getByText("$0.06")).toBeInTheDocument();
  });

  it("a historical trace without cost_usd shows '—', never $0.00", () => {
    const { cost_usd: _omitted, ...statsWithoutCost } = TRACE.stats;
    traceData = { ...TRACE, stats: statsWithoutCost as RunTrace["stats"] };
    renderWithIntl(<RunTraceDrawer runId="r1" agentName="Security" prNumber={482} onClose={() => {}} />);
    const label = screen.getByText("COST");
