...
export default async function pullsRoutes(appBase: FastifyInstance) {
  ...
  app.get('/repos/:id/pulls', { schema: { params: IdParams } }, async (req): Promise<PrMeta[]> => {
    ...
    // Per-PR LLM cost = SUM of the PR's agent_runs.cost_usd. SQL SUM ignores
    // NULLs and returns NULL when no run has a cost — so a PR without cost data
    // stays null ("—" in the UI), never a fabricated $0.00.
    ...
    if (prIds.length > 0) {
      ...
      for (const row of sevRows) {
        const bucket = grouped.get(row.prId);
        if (bucket) bucket.push(row);
        else grouped.set(row.prId, [row]);
      }
      for (const [prId, bucket] of grouped) sevByPr.set(prId, foldSeverityRows(bucket));
    }
    ...
    return rows.map((r) => {
      ...
      return {
        ...
        findings_by_severity: review ? sevByPr.get(r.id) ?? emptySeverityCounts() : null,
      };
    });
  });
